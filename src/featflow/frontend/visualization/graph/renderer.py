...
class GraphRenderer:
    ...
    def create_figure(self, graph_data: GraphData, 
        ...
        if graph_data.feature_list_intersection is not None:
            ...
            for pos, layer, idx in graph_data.feature_list_intersection:
                for node_i, node in enumerate(graph_data.nodes):
                    # Skip logit nodes for intersection feature matching
                    if node.pos == "logit" or node.layer == "logit":
                        continue
        ...
        # Create node trace with cluster coloring
        node_trace = self.node_renderer.create_node_trace(
            graph_data.nodes, 
            selected_feature_id, 
            highlighted_nodes,
        ...
        data = edge_traces + grid_traces + [node_trace] + token_labels + description_traces + logit_traces
    ...
    def _calculate_responsive_layout(self, graph_data: GraphData) -> dict:
        ...
        for pos in range(graph_data.prompt_length):
            ...
            if max_nodes_in_column == 0:
                column_width = 60  # Minimum width for empty columns
            ...
            column_widths.append(max(60, column_width))
    ...
    def _create_token_labels(self, graph_data: GraphData, layout_config: dict) -> List[go.Scatter]:
        ...
        for i in range(graph_data.prompt_length):
            if i < len(graph_data.input_tokens):
                ...
                font_size = min(12, max(8, column_width * 0.05))
                ...
                annotation = dict(
                    ...
                    textangle=-75,  # Rotate text 75 degrees for more vertical orientation
                    xanchor='center',
                    yanchor='middle'
                )
