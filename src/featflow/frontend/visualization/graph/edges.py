...
class EdgeRenderer:
    ...
    def create_edge_traces(self, nodes: List[FeatureNode], adjacency_matrix: np.ndarray,
        ...
        if edge_cache and edge_cache.is_cached():
            ...
            # FALLBACK: Use slow adjacency matrix approach
            print("⚠️  Using slow adjacency matrix edge creation")
    ...
    def _create_cached_separate_traces(self, edge_cache: EdgeCache, 
        ...
        if incoming_x:
            traces.append(go.Scatter(
                ...
                line=dict(width=3, color=self.config.incoming_edge_color),
                hoverinfo='none', showlegend=False
            ))
        ...
        if outgoing_x:
            traces.append(go.Scatter(
                ...
                line=dict(width=3, color=self.config.outgoing_edge_color),
                hoverinfo='none', showlegend=False
            ))
