...
def register_cluster_callbacks(app, graph_component: GraphComponent, cluster_manager: ClusterManager):
    ...
    def handle_intervention_dialog(run_clicks, cancel_clicks, intervention_mode_data, clusters, cluster_intervention_strength, manual_features, freeze_attention_toggle):
        ...
        if trigger_id == 'cancel-intervention-btn':
            ...
            try:
                # Submit the slurm job using the .sbatch file
                submit_script = "/home/abir19/FeatFlow/src/featflow/frontend/run_cluster/run_cluster_intervention.sbatch"
                
                # Write arguments to a file in the run_cluster folder
                temp_args_file = "/home/abir19/FeatFlow/src/featflow/frontend/run_cluster/cluster_intervention_args.json"
                args_data = {
                ...
                results_file_path = "/home/abir19/FeatFlow/src/featflow/frontend/run_cluster/cluster_intervention_results.json"
                ...
                result = subprocess.run([
                    "sbatch",
                    "--time=24:00:00",  # Adjust time limit as needed
                    f"--job-name=featflow",
                    submit_script,
                    temp_args_file
                ], capture_output=True, text=True, cwd="/home/abir19/FeatFlow")
                ...
                if result.returncode == 0:
                    ...
                    job_id = None
                    print(f"DEBUG: Parsing slurm submit output: {result.stdout}")
                    ...
                    print(f"Slurm submit failed: {result.stderr}")
                    error_content = [
                        html.Div([
    ...
    def monitor_job_progress(n_intervals, job_data):
        """Monitor Slurm job progress and update the progress dialog."""
        import subprocess
        import json
        import os
        ...
        try:
            # Check job status with squeue - retry if first call fails or returns empty
            job_found = False
            job_status = None
            ...
            for attempt in range(3):  # Try up to 3 times
                ...
                if result.returncode == 0 and result.stdout.strip():
                    print(f"DEBUG: squeue output (attempt {attempt + 1}): {result.stdout}")
                    for line in result.stdout.strip().split('\n'):
                        if line.strip():
                            parts = line.strip().split()
                            if len(parts) >= 2:
                                cluster_id = parts[0]
                                if cluster_id == str(job_id):
                                    job_found = True
                                    job_status = parts[1]  # SLURM uses letter codes (R, PD, CG, etc.)
                                    print(f"DEBUG: Found job {job_id} with status {job_status}")
                                    break
                    if job_found:
                        break  # Job found, no need to retry
                else:
                    print(f"DEBUG: squeue attempt {attempt + 1} failed or returned empty (returncode: {result.returncode})")
            ...
            if not job_found:
                ...
                results_file = "/home/abir19/FeatFlow/src/featflow/frontend/run_cluster/cluster_intervention_results.json"
                ...
                if os.path.exists(results_file):
                    ...
                    error_log_file = "/home/abir19/FeatFlow/src/featflow/frontend/run_cluster/condor_intervention.err"
                        ...
                        try:
                            with open(error_log_file, 'r') as f:
                                ...
                                if error_log.strip():
                                    ...
                                    error_details = f"Job completed but no results file found. Last error log lines:\n" + '\n'.join(error_lines)
                        except Exception as e:
