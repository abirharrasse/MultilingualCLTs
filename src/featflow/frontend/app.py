...
def main():
    """Main entry point."""
    try:
        # Configure logging to show errors but reduce Flask spam
        import logging
        log = logging.getLogger('werkzeug')
        log.setLevel(logging.WARNING)  # Show warnings and errors, not debug info
        
        app = create_app()
        config = AppConfig.from_env()
        
        # Enable debug mode for development (shows errors in browser and enables hot reload)
        debug_mode = config.debug or True  # Force debug mode for development
